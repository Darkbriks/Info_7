...
void board::print_board(type::type_board type_board)
{
    cout << "  +---+---+---+---+---+---+---+---+" << endl;
    for (int i = 7; i >= 0; i--)
    {
...
void board::write_fen(type::type_board fen, std::string file_name)
{
    ofstream file;
    file.open(file_name);
    if (file.is_open())
...
// Read and write FEN in a file
void board::read_fen(type::type_board fen, std::string file_name)
{
    ifstream file;
    file.open(file_name);
    if (file.is_open())
    {
        string fen_string;
        getline(file, fen_string);
        fen.set_fen(fen_string);
        file.close();
    }
    else
    {
